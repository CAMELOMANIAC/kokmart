...
describe('generateGeminiTipBatch', () => {
  ...
  it('검색 호출과 TSV 출처 URL이 있으면 citation annotation 없이도 팁을 생성한다', async () => {
    mockCreateInteraction.mockResolvedValueOnce({
      output_text: [
        ...
        'product-1\t6900\t6900\t행사몰\t피자 파티세트 1세트\tPRICE\t현재 공개 행사가\thttps://example.com/sale\t여럿이 먹을 양이라면 묶음 구성을 비교해 구매하기 좋아요.',
      ].join('\n'),
      steps: [
        {
    ...
    expect(result.products[0]?.smartTip?.tipMessage).toContain('묶음 구성을 비교해 구매하기 좋아요');
    expect(result.products[0]?.smartTip?.tipMessage).not.toContain('피자 파티세트:');
  });
  ...
  it('가격 비교가 없어도 NONE 행의 AI 구매조언으로 완료한다', async () => {
    mockCreateInteraction.mockResolvedValueOnce({
      output_text: [
        ...
        'product-1\tNONE\t0\t0\t-\t-\tREADY_TO_EAT\t-\t매장 자체 구성 상품\t-\t필요한 양만 고르기 좋아요.',
      ].join('\n'),
  ...
  it('CLOSE 냉동 상품의 보관·조리 문구는 구매 판단 문구로 대체한다', async () => {
    ...
    expect(result.products[0]?.smartTip?.tipMessage).toContain('온라인 묶음 구매가 더 실용적이에요.');
    expect(result.products[0]?.smartTip?.tipMessage).not.toMatch(/냉동실|조리|보관/);
  });
});
